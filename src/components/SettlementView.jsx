...
function SettlementView({ friends, balances, settlements, payments = [], expenses = [], stats = {}, categories = [], onRecordPayment, onDeletePayment, viewOnly, selectedUser }) {
    ...
    const getCategoryInfo = (categoryId) => categories.find(c => c.id === categoryId) || { icon: '📦', name: 'Other' };
    ...
    const generateShareText = () => {
        let text = `🏝️ *${TRIP_INFO.name}*\n`;
        text += `📍 ${TRIP_INFO.location} • ${TRIP_INFO.dates || `${TRIP_INFO.startDate} to ${TRIP_INFO.endDate}`}\n\n`;

        // Trip Overview
        text += `📊 *Trip Overview*\n`;
        text += `• Total Spent: ₹${Math.round(stats.totalSpent || 0).toLocaleString('en-IN')}\n`;
        text += `• Total Expenses: ${stats.expenseCount || 0}\n`;
        text += `• Per Person Avg: ₹${Math.round((stats.totalSpent || 0) / friends.length).toLocaleString('en-IN')}\n\n`;
        ...
        friends.forEach(friend => {
            ...
            if (paid > 0) {
                text += `• ${friend.name}: ₹${Math.round(paid).toLocaleString('en-IN')}\n`;
            }
        });
        text += `\n`;

        // Per Person Spending (their share)
        text += `🍽️ *Per Person Spending*\n`;
        friends.forEach(friend => {
            const spent = personSpending[friend.id] || 0;
            text += `• ${friend.name}: ₹${Math.round(spent).toLocaleString('en-IN')}\n`;
        });
        text += `\n`;
        ...
        if (expenses.length > 0) {
            ...
            sortedExpenses.forEach(exp => {
                ...
                text += `• ${cat.icon} ${exp.description}: ₹${Math.round(exp.amount).toLocaleString('en-IN')} (paid by ${payer.name})\n`;
            });
            text += `\n`;
        }
        ...
        if (stats.byCategory && Object.keys(stats.byCategory).length > 0) {
            ...
            categories.forEach(cat => {
                ...
                if (amount > 0) {
                    text += `• ${cat.icon} ${cat.name}: ₹${Math.round(amount).toLocaleString('en-IN')}\n`;
                }
            });
            text += `\n`;
        }
        ...
        if (settlements.length === 0) {
            text += `🎉 All settled up! No pending payments.\n`;
        } else {
            text += `💸 *Settlements Required*\n`;
            settlements.forEach(t => {
                const from = getFriendInfo(t.from);
                const to = getFriendInfo(t.to);
                text += `• ${from.name} ➜ ${to.name}: ₹${Math.round(t.amount).toLocaleString('en-IN')}\n`;
            });
        }
        ...
        if (payments.length > 0) {
            ...
            payments.forEach(p => {
                ...
                text += `• ${from.name} paid ${to.name}: ₹${Math.round(p.amount).toLocaleString('en-IN')}\n`;
            });
        }
        ...
        <div className="settlement-view">
            ...
            <section className="section trip-summary-section">
                ...
                <div className="person-breakdown">
                    <h3 className="subsection-title">💳 Per Person Breakdown</h3>
                ...
                {expenses.length > 0 && (
                    <div className="all-expenses">
                        <h3 className="subsection-title">🧾 All Transactions ({expenses.length})</h3>
            ...
            <section className="section">
                ...
                {settlements.length === 0 ? (
                    <div className="all-settled">
                        <div className="all-settled-icon">🎉</div>
                                ...
                                <div key={payment.id} className="payment-item" style={{ animationDelay: `${index * 0.05}s` }}>
                                    <div className="payment-info">
                                        <div className="payment-avatars">
                                            ...
                                            <span className="payment-arrow-small">→</span>
                                        ...
                                        <div className="payment-details">
                                            ...
                                            <span className="payment-meta">{payment.note} • {payment.date}</span>
                                        </div>
                                    </div>
